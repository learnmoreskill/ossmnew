...
$account = new account_management_classes();
 ...
 if(isset($_GET['edit_id']) && !empty($_GET['edit_id']))
 {
  ...
  $last_payment_month = $last_payment_month + 1; 
 }
?>
<script type="text/javascript">
  $(document).ready(function() {
    ...
    if(isNaN(bonus))
    {
      ...
      var net_balane = salary + bonus - deducted;
...
<form id='form' method='post'  name="teacher_payment_form">
  ...
  <div class="form-row" style="padding-top: 0px;">
    <div class="col-md-6" style="margin-top: 10px;">
      ...
      <?php
      echo "<input type='text' readonly='true' value='".$account->get_english_month($last_payment_month)."' class='form-control' name='first_date' >";
      ...
      <select onchange='OnSelectionChange (this)'  name='second_date' id='last_payment_date' class='form-control'>
        ...
        <?php
        $m = date('m');
         for($i=$last_payment_month; $i<=$m;$i++)
          {
            echo "<option>".$account->get_english_month($i)."</option>";
          }
         ?>
        </select>
    </div>
  ...
  <div class="form-row">
     ...
     <div class="form-group col-md-4" style="margin-top: 10px;">
      ...
      <input readonly="true" type="text" required="" class="form-control" name="paying_amount" id="paying_amount" value='0'>
    </div>
...
function OnSelectionChange (select) 
{
  ...
  var first_date = teacher_payment_form.first_date.value;
  var second_date = select.options[select.selectedIndex].value;
