...
if(isset($_REQUEST['second_month']))
{
	...
	$f_month_number = $account->get_english_month_num($first_month);
	$s_month_number = $account->get_english_month_num($second_month);
...
if(isset($_REQUEST['add_due_date']))
{
	...
	$selected_month = date("m",strtotime($add_due_date));

	$current_year = date("Y",strtotime($current_date));
	$selected_year = date("Y",strtotime($add_due_date));
	...
	else
	{
		...
		$fee_rate = $account->get_fee_by_class_name($student_id,$feetypelower);
	}
...
if(isset($_REQUEST['edit_due_date']))
{
	...
	$current_month = date("m",strtotime($current_date));
	$selected_month = date("m",strtotime($edit_due_date));

	$current_year = date("Y",strtotime($current_date));
	$selected_year = date("Y",strtotime($edit_due_date));
	$diff_month = $current_month - $selected_month;
	$diff_year = $current_year - $selected_year;
	$message = '';
	if($diff_year>0)
