...
if (isset($_GET["token"]) && $_GET["token"] == "amu8x008"){
        ...
        if (empty($month_id)) { $month_id=0; }
            ...
            $resultaddedmark = $db->query("SELECT `marksheet`.`marksheet_id`,`studentinfo`.`sid`,`studentinfo`.`sroll`,`studentinfo`.`sname`,`marksheet`.`m_theory`,`marksheet`.`m_practical`,`marksheet`.`m_obtained_mark` 
            FROM `studentinfo`
            LEFT JOIN `marksheet` 
