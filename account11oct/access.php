                ...
                if($max_bill_id=='')
                  { 
                    ...
                    $status=$studentBillDetails->status;
                      ...
                      $last_update_year = $last_update_date[0] + 0;
                      $last_update_month = $last_update_date[1];
                      echo "<div style='background: black; width:100%; color: yellow;'>";
                      var_dump($last_update_year);
                      echo "</div><div style='background: black; width: 100%; margin-top:50px; color: green;'>";
                      var_dump($DATE);
                      echo "</div>";
                      //new checkt
                      $month_diff = ($current_year - $last_update_year) + ($current_month - $last_update_month);
                        ...
                        if($month_diff >= 1 && $status =='Pending')
                          {
                            ...
                            $account->update_balance_by_bill_id($balance,$max_bill_id);
                          }
                        if($month_diff >= 1 && $status =='Advance')
                          {
                            $advance_balance = $old_balance - ($monthly_fee*$month_diff);
                            $account->update_balance_by_bill_id($advance_balance,$max_bill_id);
                          }
                      }    
               }
