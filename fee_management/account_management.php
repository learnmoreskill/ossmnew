...
class account_management_classes
{
	...
	function add_activity_log($teacher_id, $teacher_role, $activity, $description, $related, $url){
		...
		$qry="INSERT INTO `activity_log` (`teacher_id`, `teacher_role`, `activity`, `description`, `related`, `ip`, `browser`, `url`, `status`) VALUES ('$teacher_id', '$teacher_role', '$activity', '$description', '$related', '$ip', '$browser', '$url', 1)";
		$result = mysqli_query($db, $qry);
		mysqli_close($db);
		return $result;
	}
	//========================== end activity log tables ==========================
...
//============================== student record section =======================
	...
	function get_student_details_by_year_id($year_id)
	{
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT `studentinfo`.* , `parents`.`spname`, `parents`.`smname`, `parents`.`spnumber`, `class`.`class_name`, `section`.`section_name` 
			...
			WHERE `studentinfo`.`batch_year_id` = '$year_id' AND `studentinfo`.`status`=0");
...
//================== Student transaction =======================================
	...
	function get_advance_amount_from_student_transaction_by_student_id($std_id){
        ...
        $result = mysqli_query($db,	"SELECT `advance` 
        	FROM `student_transaction` 
        	WHERE `student_transaction`.`std_id` = '$std_id' AND `student_transaction`.`status` = 1 
	...
	function get_debit_student_transaction_list_by_bill_id($bill_print_id){
		...
		$result = mysqli_query($db, "SELECT `student_transaction`.* ,`student_due`.`feetype_id`, `student_due`.`balance` , `student_due`.`date` AS `balance_date` ,`fee_types`.`feetype_title`  
			...
			WHERE bill_print_id='$bill_print_id' AND `student_transaction`.`status` = 1 AND type=0");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"id" => $row['id'],
	...
	function get_debit_student_transaction_list_group_by_feetype_by_bill_id($bill_print_id){
		...
		$result = mysqli_query($db, "SELECT 
			...
			INNER JOIN `student_due` ON `student_transaction`.`student_due_id` = `student_due`.`id`
			LEFT JOIN `fee_types` ON `student_due`.`feetype_id` = `fee_types`.`feetype_id` 
			WHERE `student_transaction`.`bill_print_id` = '$bill_print_id' AND `student_transaction`.`type` = 0 AND `student_transaction`.`status` = 1 GROUP BY `student_due`.`feetype_id` ORDER BY `student_due`.`feetype_id`, `student_due`.`date`	");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"debit" => $row['debit'],
	...
	function get_credit_student_transaction_list_by_bill_id($bill_print_id){
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT * FROM `student_transaction` WHERE bill_print_id='$bill_print_id' AND `student_transaction`.`status` = 1 AND type=1");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"id" => $row['id'],
	...
	function get_statement_by_student_id_single_date($student_id,$date){
		...
		$result = mysqli_query($db, "SELECT `student_transaction`.* ,`student_due`.`feetype_id`, `student_due`.`balance` , `student_due`.`date` AS `balance_date` , `fee_types`.`feetype_title` 
			...
			LEFT JOIN `fee_types` ON `student_due`.`feetype_id` = `fee_types`.`feetype_id` 
			WHERE `student_transaction`.`std_id` = '$student_id' AND YEAR(`student_transaction`.`date`) LIKE '$date' AND `student_transaction`.`status` = 1
			ORDER BY `student_transaction`.`timestamp` ASC,`student_transaction`.`type` ASC,`student_transaction`.`id` ASC ");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
	...
	function get_student_statement_by_student_id_and_twodate($student_id,$first_date,$second_date)
	{
		...
		$result = mysqli_query($db, "SELECT `student_transaction`.* ,`student_due`.`feetype_id`, `student_due`.`balance` , `student_due`.`date` AS `balance_date` , `fee_types`.`feetype_title` 
			...
			LEFT JOIN `student_due` ON `student_transaction`.`student_due_id` = `student_due`.`id`
			LEFT JOIN `fee_types` ON `student_due`.`feetype_id` = `fee_types`.`feetype_id` 
			WHERE `student_transaction`.`std_id` = '$student_id' AND `student_transaction`.`date` >= '$first_date' AND `student_transaction`.`date` <='$second_date' AND `student_transaction`.`status` = 1
			ORDER BY `student_transaction`.`timestamp` ASC,`student_transaction`.`type` ASC,`student_transaction`.`id` ASC ");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
	...
	function get_day_book_credit_with_class_group_by_date($date){
		...
		$result = mysqli_query($db, "SELECT `class`.`class_id`, `class`.`class_name`, SUM(`student_transaction`.`credit`) AS `income`
				FROM `student_transaction` 
				INNER JOIN `bill_tables` ON `student_transaction`.`bill_print_id` = `bill_tables`.`id` AND `bill_tables`.`status` = 1
				INNER JOIN `class` ON `bill_tables`.`class_id` = `class`.`class_id`
				WHERE `student_transaction`.`type` =1 AND `student_transaction`.`date` = '$date' AND `student_transaction`.`status` = 1
	...
	//Krishna mandal 28-03-2019
	function get_day_book_credit_details_by_date($date){
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT `studentinfo`.`sid`,`studentinfo`.`sroll`,`studentinfo`.`sname`, `student_transaction`.`credit` AS `income`,`student_transaction`.`bill_print_id`,`bill_tables`.`bill_type`,`bill_tables`.`bill_number`
			, `class`.`class_id`, `class`.`class_name`,`section`.`section_name`
...
//================== Bill Tables =======================================
	...
	//nsk february 2019
	function generate_new_bill_number($schoolCode,$billType)
	{
		if ($billType == "payment") {
			$max_bill_table_id = $this->get_max_bill_table_id($billType);
			...
			$max_bill_table_id = $this->get_max_bill_table_id($billType);
    		$new_bill_table_id = $max_bill_table_id+1;
    		$bill_number = strtolower($schoolCode).'-'.'oi'.'-'.$new_bill_table_id;
		}else if($billType == "expense") {
			$max_bill_table_id = $this->get_max_bill_table_id($billType);
    		$new_bill_table_id = $max_bill_table_id+1;
    		$bill_number = strtolower($schoolCode).'-'.'ex'.'-'.$new_bill_table_id;
		}
	...
	//nsk 29January2019
	function get_max_bill_table_id($billType)
	{
		require("../config/config.php");
		if ($billType == "payment") {

			$query = "SELECT `id` ,`bill_number` FROM `bill_tables` WHERE (`bill_type` = 0 OR `bill_type` = 1) ORDER BY `id` DESC LIMIT 0,1";
		
		}else if ($billType == "income") {

			$query = "SELECT `id` ,`bill_number` FROM `bill_tables` WHERE `bill_type` = 3  ORDER BY `id` DESC LIMIT 0,1";
		
		}else if ($billType == "expense") {

			$query = "SELECT `id` ,`bill_number` FROM `bill_tables` WHERE `bill_type` = 2 ORDER BY `id` DESC LIMIT 0,1";
		}
		...
		$exploded = explode('-', $bill_number);
		$bill_number = end($exploded);
	...
	function get_bill_details_by_bill_id($bill_id){
		...
		$result = mysqli_query($db, "SELECT `bill_tables`.* , `principal`.`pname` , `staff_tbl`.`staff_name`, `class`.`class_name`, `section`.`section_name`
			...
			LEFT JOIN `section` ON `bill_tables`.`section_id` = `section`.`section_id`
			WHERE `bill_tables`.`id` = '$bill_id' AND `bill_tables`.`status` = 1 ");
	...
	//hackster 24/jan/2019
	function get_bill_details_of_income_by_bill_id($bill_id){
		require("../config/config.php");
	...
	//hackster krishna june 2019
	function delete_bill_by_id($bill_id)
	{
		require("../config/config.php");
		$qry="UPDATE `bill_tables` SET `status`=0 WHERE `id` = '$bill_id'";
		$result = mysqli_query($db, $qry);
		mysqli_close($db);
		return $result;
	}
...
//================== Student Due =======================================
	...
	//hacksterkrishna 3/feb/2019
	function update_student_due_details_by_id($id, $balance, $description, $current_date, $LOGIN_CAT, $LOGIN_ID){
		require("../config/config.php");
		$result = mysqli_query($db, "UPDATE `student_due` SET `balance` = '$balance' , `description` = '$description' , `date` = '$current_date' , `t_role` = '$LOGIN_CAT' , `t_id` = '$LOGIN_ID'  WHERE `id`='$id'");
	...
	//hacksterkrishna 6/June/2019
	function delete_student_due_by_id($id){
		require("../config/config.php");
		$result = mysqli_query($db, "UPDATE `student_due` SET `live_status` = 0  WHERE `id`='$id'");
	...
	function student_due_fee_exist_id_by_student_id_feetype_id($sid,$feetype_id){
		require("../config/config.php");
		$result = mysqli_query($db, "SELECT `id`
			FROM `student_due`
			WHERE `student_due`.`std_id` = '$sid' AND `student_due`.`feetype_id` = '$feetype_id' AND `student_due`.`status` = 1 AND `student_due`.`live_status` = 1 ");
	...
	function get_student_due_balance_if_exist_by_student_id_feetype_id($sid,$feetype_id){
		require("../config/config.php");
		$result = mysqli_query($db, "SELECT `balance`
			FROM `student_due`
			WHERE `student_due`.`std_id` = '$sid' AND `student_due`.`feetype_id` = '$feetype_id' AND `student_due`.`status` = 1 AND `student_due`.`live_status` = 1 ");
	...
	function get_pending_active_student_due_list()
	{
		...
		$result = mysqli_query($db, "SELECT `student_due`.`std_id`
			...
			WHERE  `studentinfo`.`status` = 0 AND `student_due`.`status` = 1 AND `student_due`.`live_status` = 1
	...
	// 		FROM `student_due`
	// 		WHERE `student_due`.`std_id` = '$std_id' AND `student_due`.`status` = 1 AND `student_due`.`live_status` = 1 
	// 		GROUP BY `student_due`.`std_id` ");
	...
	function check_fee_exist_by_feetypeid_sid_year_month($feetype_id,$sid,$year,$month)
	{
		...
		$result = mysqli_query($db, "SELECT `id`  
			FROM `student_due` 
			WHERE `std_id` = '$sid' AND `feetype_id` = '$feetype_id' AND YEAR(`date`) = '$year'  AND MONTH(`date`) = '$month'");
	...
	function get_total_student_due_by_student_id_status($student_id,$status)
	{
		...
		$result = mysqli_query($db, "SELECT `student_due`.`id` , `student_due`.`std_id`, `student_due`.`feetype_id`, `fee_types`.`feetype_title` , SUM(`balance`) AS total_balance , COUNT(date) AS total_month 
			FROM `student_due` 
			LEFT JOIN `fee_types` ON `student_due`.`feetype_id` = `fee_types`.`feetype_id` 
			WHERE  `student_due`.`std_id` = '$student_id' AND `student_due`.`status` = '$status' AND `student_due`.`live_status` = 1
			GROUP BY `student_due`.`feetype_id` ");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"id" => $row['id'],
	...
	//hackster 21/jan/2019
	function get_total_student_due_details_by_student_id_status($student_id,$status)
	{
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT `student_due`.`id` , `student_due`.`feetype_id`, `fee_types`.`feetype_title` ,
		 `student_due`.`balance` , `student_due`.`description` , `student_due`.`date`
			FROM `student_due` 
			LEFT JOIN `fee_types` ON `student_due`.`feetype_id` = `fee_types`.`feetype_id` 
			WHERE  `student_due`.`std_id` = '$student_id' AND `student_due`.`status` = '$status' AND `student_due`.`live_status` = 1
			ORDER BY `student_due`.`timestamp` ");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"id" => $row['id'],
	...
	function get_due_type_details_by_feetype_id_student_id_status($feetype_id,$student_id,$status)
	{
		...
		$result = mysqli_query($db, "SELECT `student_due`.`id`, `student_due`.`std_id`, `student_due`.`feetype_id` , `balance` , `student_due`.`description` , `student_due`.`date` , `student_due`.`t_role`, `student_due`.`t_id` , `principal`.`pname` , `staff_tbl`.`staff_name`
			...
			LEFT JOIN `principal` ON `student_due`.`t_id` = `principal`.`pid`
			LEFT JOIN `staff_tbl` ON `student_due`.`t_id` = `staff_tbl`.`stid` 
			WHERE `student_due`.`feetype_id` = '$feetype_id' AND  `student_due`.`std_id` = '$student_id' AND `student_due`.`status` = '$status' AND `student_due`.`live_status` = 1 ");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"id" => $row['id'],
...
//================== Fee type section ================================================
	...
	//hackster krishna may 2019
	function check_feetype_name_exist($name){
			require("../config/config.php");
			$result = mysqli_query($db, "SELECT `feetype_id` AS `id` FROM `fee_types` WHERE `feetype_title` = '$name'");
			$count=mysqli_num_rows($result);
			mysqli_close($db);
			if($count>0){ return true; }else{ return false;}
	}
	...
	//hackster krishna may 2019
	function check_feetype_name_exist_except_id($name,$id){
			require("../config/config.php");
			$result = mysqli_query($db, "SELECT `feetype_id` AS `id` FROM fee_types WHERE `feetype_title` = '$name' AND `feetype_id` <> '$id'");
			$count=mysqli_num_rows($result);
			mysqli_close($db);
			if($count>0){ return true; }else{ return false;}
	}
	...
	function update_feetype($feetype_title,$feetype_id)
	{
		require("../config/config.php");
		$qry="UPDATE `fee_types` SET `feetype_title`='$feetype_title' WHERE feetype_id='$feetype_id'";
		$result = mysqli_query($db, $qry);
		mysqli_close($db);
		//$this->createlog($qry);
		return $result;
	...
	function get_feetype_details()
	{
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT * FROM `fee_types` WHERE `status` = 1");
		while($row = mysqli_fetch_assoc($result))
...
//================= expenses tables =================================================
	...
	//hackster krishna may 2019
	function insert_into_expenses_category($category)
	{
	...
	//hackster krishna may 2019
	function check_expenses_cat_exist($category)
		{
			require("../config/config.php");
			$result = mysqli_query($db, "SELECT `ecat_id` AS `id` FROM `expenses_cat` WHERE `exp_cat` = '$category'");
			$count=mysqli_num_rows($result);
			mysqli_close($db);
			if($count>0){ return true; }else{ return false;}
		}
	...
	//hackster krishna may 2019
	function check_expenses_cat_exist_except_id($category,$id)
		{
			require("../config/config.php");
			$result = mysqli_query($db, "SELECT `ecat_id` AS `id` FROM expenses_cat WHERE `exp_cat` = '$category' AND `ecat_id` <> '$id'");
			$count=mysqli_num_rows($result);
			mysqli_close($db);
			if($count>0){ return true; }else{ return false;}
		}
	...
	//hackster krishna may 2019
	function updateExpensesCategory($category,$ecat_id)
	{
		require("../config/config.php");
	...
	//Krishna mandal 28-03-2019
	function get_day_book_expenses_details_by_date($date){
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT `expenses_add`.* , `expenses_cat`.`exp_cat` ,`bill_tables`.`bill_type`,`bill_tables`.`bill_number`
				FROM `expenses_add` 
				INNER JOIN `bill_tables` ON `expenses_add`.`bill_print_id` = `bill_tables`.`id` AND `bill_tables`.`status` = 1
				INNER JOIN `expenses_cat` ON `expenses_add`.`ecat_id` = `expenses_cat`.`ecat_id`
				WHERE `expenses_add`.`date` = '$date' AND `expenses_add`.`status` = 1");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"exp_id" => $row['id'],
...
//================ income tables ====================================================
	function getExtraIncome()
	{
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT `incometables`.* , `bill_tables`.`id` AS `bill_id`,`bill_tables`.`bill_number`,`bill_tables`.`print_count` 
			FROM `incometables` 
	...
	//hackster krishna feb 2019
	function delete_income_by_id($income_id)
	{
		require("../config/config.php");
		$qry="UPDATE `incometables` SET `status`=0 WHERE `income_id` = '$income_id'";
		$result = mysqli_query($db, $qry);
		mysqli_close($db);
		//$this->createlog($qry);
		return $result;
	...
	//Krishna mandal 27-03-2019
	function get_day_book_total_income($date){
		// require("../config/config.php");
		// $response = array();
		// $result = mysqli_query($db, "SELECT  SUM(`income_amount`) AS `income_amount`
		// 		FROM `incometables` 
		// 		WHERE `date` = '$date' AND `incometables`.`status` = 1");
		// $value = mysqli_fetch_object($result);
		// mysqli_close($db);
		// return json_encode($value->income_amount);
	}

	//Krishna mandal 28-03-2019
	function get_day_book_income_details_by_date($date){
		require("../config/config.php");
		$response = array();
		$result = mysqli_query($db, "SELECT `incometables`.* ,`bill_tables`.`bill_type`,`bill_tables`.`bill_number`
				FROM `incometables` 
				INNER JOIN `bill_tables` ON `incometables`.`bill_print_id` = `bill_tables`.`id` AND `bill_tables`.`status` = 1
				WHERE `incometables`.`date` = '$date' ");
		while($row = mysqli_fetch_assoc($result))
		{
			array_push($response,array(
				"income_id" => $row['income_id'],
	...
	//hackster 24/jan/2019
	function get_income_details_of_by_bill_id($bill_id){
		require("../config/config.php");
		$result = mysqli_query($db, "SELECT `incometables`.*
			FROM `incometables` 
			WHERE `incometables`.`bill_print_id` = '$bill_id'");
