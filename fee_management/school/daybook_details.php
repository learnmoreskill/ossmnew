...
if(isset($_REQUEST['reuestForDaybookDetails'])){
  ...
  $daybookReportType = $_REQUEST['daybookReportType'];



  if ($daybookReportType=='studentPayment') {

    $class_id = $_GET['classId'];
    $studentClassCreditDetails = json_decode($account->get_day_book_credit_details_by_class_date($class_id,$selectedDate)); 
  
  }else if($daybookReportType=='incomeDetails'){
                      ...
                      foreach ($incomeDetails as $key){ ?> 
                        <tr>
                          ...
                          <td>
                            ...
                            <button class='btn btn-info' style='color:#fff;' onclick='view_bill(<?php echo $key->bill_print_id; ?>,<?php echo $key->bill_type; ?>,0)' ><i class="fa fa-eye"></i> View</button>
