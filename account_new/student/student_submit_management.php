...
//=================== Extra fee collection KRISHNA MAGH 2075 =====================
 ...
 if(isset($_REQUEST['advancePaymentForStudent'])){
    ...
    if (empty($errMsgArray)) {
      ...
      $max_bill_table_id = $account->get_max_bill_table_id();
      $new_bill_table_id = $max_bill_table_id+1;
      $bill_number = strtolower($school_details->school_code).'-'.$new_bill_table_id;
      ...
      $bill_print_id = $account->add_into_bill_tables($bill_number, 0, $due_before, $advance_before, 0, $payable_amount_after, $advance, $date, 0, $LOGIN_CAT , $LOGIN_ID);
...
if(isset($_REQUEST['submit_fee']))
{
  ...
  if (empty($errMsgArray)) {
    ...
    $bill_print_id = $account->add_into_bill_tables($bill_number, 1, $due_before, $advance_before, $advance_paid, $due_after, $advance_after, $date, 0, $LOGIN_CAT , $LOGIN_ID);
