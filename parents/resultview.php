        ...
        <main>
            <div class="section no-pad-bot" id="index-banner">
                ...
                <div class="github-commit">
                    <div class="container">
                        <div class="row center"><a class="white-text text-lighten-4" href="#">View Report Card</a></div>
                    </div>
                </div>
            </div>
            <?php 
            if ($resultclass1->num_rows > 0) {
                ...
                $class_name=$rowclass1["class_name"];
                ?>
                <div id="<?php echo $class_id; ?>" style="display: none" class="card teal center lighten-2">
                    <span class="card-title white-text">Marksheet for class:<?php echo $class_name;?> is available</span>
                </div>
                <?php

                foreach ($students as $per_student) {

                    $sqlmark1 = "SELECT * FROM `marksheet` WHERE `mstudent_id`='$per_student->sid' AND `marksheet_class`='$class_id' AND `marksheet_status` = 0";
                            ...
                            if ($resultmark1->num_rows > 0) {
                             ...
                             <div class="row">
                                <div class="col s12">
                                    <script>
                                        document.getElementById("<?php echo $class_id; ?>").style.display="block";
                                    </script>
                                ...
                                if ($resultexam1->num_rows > 0) {
                                    while($rowexam1 = $resultexam1->fetch_assoc()) {
                                        ...
                                        $sqlmark2 = "SELECT `marksheet`.`mstudent_id`,`marksheet`.`year_id`,`marksheet`.`month`,`marksheet`.`marksheet_class` ,`academic_year`.`single_year`
                                            ...
                                            WHERE `mstudent_id`='$per_student->sid' 
                                ...
                                while($rowmark2 = $resultmark2->fetch_assoc()) { ?>
                                    ...
                                    <div class="row center mb-0">
                                        <a class="col s12 m6 offset-m3 btn waves-effect waves-light blue lighten-2" href="marksheet.php?token=3drtgy&sid=<?php echo $rowmark2["mstudent_id"]; ?>&eid=<?php echo $examid; ?>&year_id=<?php echo $rowmark2["year_id"]; ?>&month=<?php echo $rowmark2["month"]; ?>&class_id=<?php echo $rowmark2["marksheet_class"]; ?>">
                                            <?php echo $examname; if (!empty($rowmark2["month"]) || $rowmark2["month"]!=0) { echo ' ( '.$months[$rowmark2["month"]-1].' ) '; }  ?> Marksheet for child:<?php echo $per_student->sname;?>
                                        </a>
                                    </div><br>
                ...
                <div class="col s12 m12">
                    <div class="card">
                        <div class="card-panel grey darken-3">
                            ...
                            No report card found for your children.
                            </div>
                                    </div>
                                </div>
                            </div>
